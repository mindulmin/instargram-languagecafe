...
function resolveState(before, current, ledger, evidence) {
  ...
  const expected = [SEED, 'operations/cloud-controller/current.json', 'operations/cloud-controller/history.json', 'operations/cloud-controller/policy.json'].sort();
  const added = Object.keys(current.files).filter(p => !(p in before.files)).sort();
  if (JSON.stringify(added) !== JSON.stringify(expected)
    || Object.keys(before.files).some(p => current.files[p] !== before.files[p])) fail();
...
async function main() {
  ...
  if (current.sha !== EXPECTED_STATE) throw Error('State head changed; fresh operator investigation required');
  ...
  const job = failedJobs.jobs.find(j => j.id === 103193387008);
  ...
  if (failed.status !== 'completed' || preview.status !== 'completed' || preview.conclusion !== 'success' || native?.conclusion !== 'success'
    || failed.head_sha.slice(0, 7) !== '9c04bb0' || !job?.steps.some(s => s.name === 'Produce and publish one authorized learning pair' && s.conclusion === 'failure')) throw Error('Hosted run evidence not verified');
  ...
  const evidence = { failedRun: FAILED_RUN, failedRunConclusion: failed.conclusion, checkedAt: new Date().toISOString(), nativePreviewRun: previewId,
    nativeSpawnVerified: true, failedLogSha256: digest(log), agentFailedBeforeSpawn: /Error: spawn codex ENOENT/.test(plainLog) && /syscall: 'spawn codex'/.test(plainLog),
  ...
  const resolved = resolveState(before, state, current.ledger, evidence);
