...
2026-09-11 공식 실조회: 최근 캐러셀 3건의 인사이트 모두 권한 오류로 unavailable이다. 도달 GET은 HTTP 400 / Meta code 10, `Application does not have permission for this action`을 반환했다. 연결 앱의 인사이트 권한 확인과 계정 소유자의 재인증이 선행돼야 실제 반응 수치를 모을 수 있다. 토큰 값을 채팅에 보내지 않으며, 새 세션은 기존 GitHub Secret 경로로 갱신한다. 게시 권한과 인사이트 권한을 구분한다.
