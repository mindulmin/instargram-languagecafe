...
async function prepare() {
  ...
  for (const relative of tracked) {
    if (relative.startsWith('.github/') || relative.startsWith('cloud/control/')) continue;
  ...
  // Controller bytes are copied exactly, never regenerated to manufacture authorization.
  const bytes = fs.readFileSync(path.join(ROOT, 'cloud/control/current-test.json'));
  ...
  const result = { checkedAt: new Date().toISOString(), mode, status: gate.eligible ? 'eligible' : 'blocked_preflight',
    ...
    reelActionCount: 0, socialApiCalls: 0 };
  output('run_agent', 'false');
  if (mode === 'preflight' || !gate.eligible) { summarize(result); return; }
  ...
  const claimed = await stateApi.save(remote, { ...remote.ledger, lock,
    actions: [...remote.ledger.actions, lock] }, remote.encrypted);
...
async function finalize() {
  ...
  const ledger = { ...remote.ledger, lock: success ? null : { ...remote.ledger.lock, status: 'needs_official_readback' },
    lastRun: { runId: process.env.GITHUB_RUN_ID, completedAt: new Date().toISOString(),
      status: success ? 'published_learning_pair_exactly_once' : 'blocked_ambiguous_or_incomplete' } };
